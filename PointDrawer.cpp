...
XnVPointDrawer::XnVPointDrawer(XnUInt32 nHistory, xn::DepthGenerator depthGenerator) :
	...
	m_nHistorySize(nHistory), m_DepthGenerator(depthGenerator), m_bDrawDM(false), m_bFrameID(false),texture(0)
{
...
GLfloat texcoords[8];
GLfloat imgTexcoords[8];
void DrawRectangle(float topLeftX, float topLeftY, float bottomRightX, float bottomRightY,float z)
{
  if(z == 0.0)
  {
  ...
  {
    GLfloat verts[12] = {	topLeftX, topLeftY,z,
      topLeftX, bottomRightY,z,
      bottomRightX, bottomRightY,z,
      bottomRightX, topLeftY,z
    };
...
void DrawTexture(float topLeftX, float topLeftY, float bottomRightX, float bottomRightY, GLfloat* tex_coordinates, float z = 0.0)
{
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	glTexCoordPointer(2, GL_FLOAT, 0, tex_coordinates);

	DrawRectangle(topLeftX, topLeftY, bottomRightX, bottomRightY, z);
...
void DrawDepthMap(const xn::DepthMetaData& dm)
{
	...
	DrawTexture(dm.XRes(),dm.YRes(),0,0,texcoords);	
...
XnFloat Colors[][3] =
{
	...
	{1,0.5,0},	// Orange
	{1,0,1},	// Purple
	{0,1,0}		// Green, reserved for primary point 
};
XnUInt32 nColors = 6;
...
void XnVPointDrawer::Draw() const
{
  ...
  //PointIterator--;
  XnPoint3D ptProjective(ptPosition);
  //XnPoint3D ptProjective(cxt->ptPosition)
  printf("x = %f, y = %f, z = %f\n",ptProjective.X,ptProjective.Y,ptProjective.Z);
	m_DepthGenerator.ConvertRealWorldToProjective(1, &ptProjective, &ptProjective);
  DrawImage(ptProjective.X,ptProjective.Y, ptProjective.Z);
  printf("x = %f, y = %f, z = %f\n",ptProjective.X,ptProjective.Y,ptProjective.Z);
  //DrawImage(ptPosition.X,-ptPosition.Y, ptPosition.Z);
}

void XnVPointDrawer::DrawImage(float x, float y, float z) const
{
	glEnable(GL_TEXTURE_2D);
	int tmpw =  getClosestPowerOfTwo(imgWidth);
	int tmph = getClosestPowerOfTwo(imgHeight);

	float tmpXpos =1.0f;
	//float tmpXpos =((float)imgWidth)/tmpw;
	float tmpYpos  =1.0f;
	//float tmpYpos  =((float)imgHeight)/tmph;

	memset(imgTexcoords, 0, 8*sizeof(float));
	imgTexcoords[0] = tmpXpos, imgTexcoords[1] = tmpYpos, imgTexcoords[2] = tmpXpos, imgTexcoords[7] = tmpYpos;

  glBindTexture(GL_TEXTURE_2D,texture);
  //glPushMatrix();
  //glRotatef(45.0f,0.0f,0.0f,1.0f);
  //DrawRectangle(1.0,1.0,400.0,400.0);
  float topx=0.0,topy=0.0,botx=1.0,boty=imgHeight/((float)imgWidth);
  float scale=40000/z;
  glColor4f(1.0,1.0,1.0,1.0);
  printf("Good");
  y=(y-10);
  x=(x-15);
  DrawTexture(x+(topx),y+(topy),x+(scale*botx),y+(scale*boty),imgTexcoords);
